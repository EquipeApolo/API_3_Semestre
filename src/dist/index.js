...
function dadosInput() {
    ...
    if (flapNumero == 220) {
        calculoPouso = new app_js_1.FLAP220(valuex, weight, altitude, valueTypeWind, temperature, valueTypeSlope, speedAdditive, reversor, IceAccreationCheck, 2000, wind, slope);
    }
    else if (flapNumero = 450) {
        calculoPouso = new app_js_1.FLAP450(valuex, weight, altitude, valueTypeWind, temperature, valueTypeSlope, speedAdditive, reversor, IceAccreationCheck, 2000, wind, slope);
    }
    else {
        console.log("ERRO!");
    }
